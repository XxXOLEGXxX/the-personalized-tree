var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="getFactorialEquilavent().lt('1e1000')||player.points.lt('1e1000')"  class="overlayThing">You have </span>
		<h2  class="overlayThing" id="points">{{format(getFactorialEquilavent())}}</h2>
		<span v-if="getFactorialEquilavent().lt('1e1e6')"  class="overlayThing"> factorified points and</span>
		<h2  class="overlayThing" id="points">{{format(player.points)}}</h2>
		<span v-if="player.points.lt('1e1e6')"  class="overlayThing"> {{modInfo.pointsName}}</span>
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}} points/sec)</span>
